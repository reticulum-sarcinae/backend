...
public class EventControllerMockMvcTest {
  ...
  void test_delete() throws Exception {
    ...
    mockMvc.perform(
        delete("/api/event/{eventId}", event.getId())
      )
      .andExpect(status().isNoContent());

    assertThat(eventRepository.findById(event.getId())).isEmpty();
    assertThat(eventParticipantRepository.findById(participant.getId())).isEmpty();
  }

  @Test
  void test_add_participant() throws Exception {
    var event = eventRepository.save(
      EventEntity.builder()
        .name("BREEZE")
        .startTime(OffsetDateTime.of(LocalDate.of(2027, 8, 18).atStartOfDay(), ZoneOffset.UTC))
    ...
    mockMvc.perform(
        post("/api/event/{eventId}/addParticipant", event.getId())
          .contentType(MediaType.TEXT_PLAIN)
          .content("Wirtshausfranz")
      )
      .andExpect(status().isOk())
      .andExpect(content().json(
        """
    ...
    assertThat(eventParticipantRepository.findAllByEvent(event))
      .singleElement()
      .satisfies(participantEntity -> assertThat(participantEntity.getName()).isEqualTo("Wirtshausfranz"));
  }

  @Test
  void test_delete_participant() throws Exception {
    var event = eventRepository.save(
      EventEntity.builder()
        .name("BREEZE")
        .startTime(OffsetDateTime.of(LocalDate.of(2027, 8, 18).atStartOfDay(), ZoneOffset.UTC))
    ...
    var participantToDelete = eventParticipantRepository.save(
      EventParticipantEntity.builder()
        .name("Wirtshausfranz")
        .event(event)
        .build()
    );
    var participantToKeep = eventParticipantRepository.save(
      EventParticipantEntity.builder()
        .name("Konterbierkarl")
        .event(event)
        .build()
    ...
    mockMvc.perform(
        delete("/api/event/{eventId}/deleteParticipant", event.getId())
          .contentType(MediaType.APPLICATION_JSON)
          .content(objectMapper.writeValueAsString(participantToDelete.getId()))
      )
      .andExpect(status().isOk())
      .andExpect(content().json(
        """
          ...
          """.formatted(event.getId(), participantToKeep.getId())
      ));

    assertThat(eventParticipantRepository.findAllByEvent(event))
      .singleElement()
      .satisfies(participantEntity -> {
        assertThat(participantEntity.getId()).isEqualTo(participantToKeep.getId());
        assertThat(participantEntity.getName()).isEqualTo("Konterbierkarl");
      });
    assertThat(eventParticipantRepository.findById(participantToDelete.getId())).isEmpty();
  }
}
